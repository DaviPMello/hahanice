...
void eventCheck(SDL_Window* mainWindow, bool &end, bool &rain, bool &restart, int &highscore, int carState, Menu &menu, SDL_Point mouse, Screen &screen){
    SDL_Event gameEvent;
...
void restartVars(bool &restart, int &carState, Road &road, NPCCar (&npcCar)[2], CarPiece &carHood, CarPiece (&pieces)[3], int &score, SDL_Point &lamp, Night &night, Rain &rain){
    if(restart == true){
        carState = 3;
